...
(function(){
  ...
  async function start(){
    addMsg('👋 Welcome! We\'ll set up your family tree in chat.');
  ...
  async function step1(){
    addMsg('Let\'s create your family.');
    while(true){
      const name = await ask('What is your family name?');
      if (!name){ addMsg('Please enter a name.'); continue; }
  const check = await postForm('/api/family_check_name.php', { name });
      if (!check.ok){ addMsg('Server is busy. Try again.'); continue; }
      if (!check.unique){ addMsg('⚠️ That family name already exists. Try another.'); continue; }
  const create = await postForm('/api/family_create.php', { name });
      if (!create.ok){ addMsg('Unable to save right now. Try again.'); continue; }
      state.family.name = name; state.family.desc = ''; setStep(2);
      addMsg(`✅ Family <b>${name}</b> created.`);
      break;
    }
  }
  ...
  async function step2(){
    ...
    addMsg('This is your registered full name');
    let selfName = '';
    try {
      ...
      if (selfName) {
        addMsg(`Name on file: <b>${selfName}</b>`);
      }
    } catch(_) { /* fallback to prompt */ }
    if (!selfName) {
      // Fallback to prompt if not available from session
      selfName = await ask('Your full name?');
    }
    let gender = await ask('Gender?', ['male','female']);
  ...
  const save = await postForm('/api/step2_self_save.php', payload);
    if (!save.ok){ addMsg('Unable to save your info. Try again.'); return; }
    state.self = { name:selfName, gender, status:'living' }; setStep(3);
    addMsg(`✅ Saved ${selfName}.`);
  }

  // STEP 3 - Parents
  async function step3(){
    addMsg('Let\'s add your parents.');
    const father = await ask('Father\'s full name?');
    const fatherStatus = await ask('Father status?', ['living','deceased','skip']);

    const mother = await ask('Mother\'s full name?');
    const motherStatus = await ask('Mother status?', ['living','deceased','skip']);
  ...
  const res = await fetch('/api/step3_parents_save.php', { method:'POST', body: fd });
    ...
    if (!j.ok){ addMsg('Unable to save parents. Try again.'); return; }
    state.parents.father = { name: father, status: fatherStatus };
    state.parents.mother = { name: mother, status: motherStatus }; setStep(4);
    addMsg('✅ Parents saved.');
  }

  // STEP 4 - Siblings
  async function step4(){
    const has = await ask('Do you have siblings?', ['yes','no']);
    if ((has||'').toLowerCase().startsWith('y')){
      let countStr = await ask('How many siblings? (number)');
      let count = parseInt(countStr, 10); if (!Number.isFinite(count) || count<1) count = 1;
      const siblings = [];
      for (let i=0;i<count;i++){
        const n = await ask(`Sibling ${i+1} name:`);
        const g = await ask(`Sibling ${i+1} gender:`, ['male','female']);
        const st = await ask(`Sibling ${i+1} status:`, ['living','deceased','skip']);
        siblings.push({
          name:n,
  ...
  const r = await fetch('/api/step4_siblings_save.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ siblings }) });
      ...
      if (!j.ok){ addMsg('Unable to save siblings. Try again later.'); return; }
      state.siblings = siblings; addMsg('✅ Siblings saved.');
    } else { addMsg('Okay, skipping siblings.'); }
    setStep(5);
  }

  // STEP 5 - Partners
  async function step5(){
    const has = await ask('Do you have a partner?', ['yes','no']);
    let partners = [];
    if ((has||'').toLowerCase().startsWith('y')){
      const n = await ask('Partner\'s name:');
      const g = await ask('Partner\'s gender:', ['male','female']);
      const st = await ask('Partner status:', ['living','deceased','skip']);
      partners.push({
  ...
  const r = await fetch('/api/step5_partners_save.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ partners }) });
      const j = await r.json().catch(()=>({ok:false}));
      if (!j.ok){ addMsg('Unable to save partner. Try again later.'); return; }
      addMsg('✅ Partner saved.');
    } else {
  await fetch('/api/wizard_complete_no_partners.php', { method:'POST' });
      addMsg('Skipping partner section.');
    }
  ...
  addMsg('🎉 Wizard complete! Redirecting to dashboard…'); await sleep(800); window.location.href='/pages/en/dashboard.html'; return;
    ...
    const children = [];
    for (let i=0;i<count;i++){
      const n = await ask(`Child ${i+1} name:`);
      const g = await ask(`Child ${i+1} gender:`, ['male','female']);
      const st = await ask(`Child ${i+1} status:`, ['living','deceased','skip']);
      children.push({
        name:n, gender:(g||'').toLowerCase(), status: st==='skip'?'':st,
        rel_type:'biological'
      });
