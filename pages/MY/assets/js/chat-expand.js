(function(){
  ...
  async function main(){
    const pid = qs('pid');
  if (!pid){ addMsg('ID individu tiada. Buka dari paparan pokok.', 'bot'); return; }

    typing(true);
    const st = await startExpand(pid).catch(()=>null);
    typing(false);
  if (!st || !st.ok){ addMsg('Tidak dapat memulakan perluasan untuk individu ini.', 'bot'); return; }
    ...
    if (pf && pf.ok){
      ...
      if (father.name || mother.name){
        const fLine = father.name ? `Bapa dalam rekod: <b>${esc(father.name)}</b>` : '';
        const mLine = mother.name ? `Ibu dalam rekod: <b>${esc(mother.name)}</b>` : '';
        addMsg([fLine,mLine].filter(Boolean).join('<br>') || 'Tiada ibu bapa dalam rekod lagi.');
      }
    }

    // CHANGED: show target person’s name
    const focusName = await fetchPersonName(pid);
  addMsg(`Mari tambah ibu bapa kepada: <b>${esc(focusName || ('#'+pid))}</b>.`);

    // Father
  let fName = father.name || await ask('Nama penuh bapa?');
    if (!fName) fName = '';
    let fStatus = (father.status||'').toLowerCase();
    if (!['living','deceased',''].includes(fStatus)){
      fStatus = await ask('Status bapa?', ['living','deceased','skip']);
    } else if (!fStatus) {
      fStatus = await ask('Status bapa?', ['living','deceased','skip']);
    }
  ...
  const fGender = 'male';
    ...
    // Mother
  let mName = mother.name || await ask('Nama penuh ibu?');
    if (!mName) mName = '';
    let mStatus = (mother.status||'').toLowerCase();
    if (!['living','deceased',''].includes(mStatus)){
      mStatus = await ask('Status ibu?', ['living','deceased','skip']);
    } else if (!mStatus) {
      mStatus = await ask('Status ibu?', ['living','deceased','skip']);
    }
  ...
  setTimeout(()=>{ window.location.href=`/pages/my/chat-expand-siblings.html?pid=${encodeURIComponent(pid)}`; }, 700);
  }
