...
(function(){
  ...
  const STATUS  = [ {label:'Hidup',value:'living'}, {label:'Meninggal',value:'deceased'}, {label:'Langkau', value:'__skip'} ];
  ...
  async function run(){
  if (!pid){ bot('ID individu tiada.'); return; }
    const pre = await loadPerson();
  if (!pre || !pre.ok){ bot('Tidak dapat memuatkan individu.'); return; }
    const p = pre.person || {};
    // Header message
  bot(`Menyunting <strong>${escapeHtml(p.full_name||('#'+pid))}</strong>. Anda boleh kekalkan nilai sedia ada dengan pilihan Langkau/Kekal.`);

    // Photo preview + upload option
    (function renderPhotoRow(){
  const current = p.photo_site ? `<div style="margin-top:8px"><img src="${escapeHtml(p.photo_site)}" alt="Foto" style="max-width:160px;border-radius:10px;border:1px solid #e2e8f0;"/></div>` : '<div class="muted" style="margin-top:6px">Tiada foto ditetapkan.</div>';
  const m = bot(`Foto${current}`);
      const chips = document.createElement('div'); chips.className='chips';
  const up = document.createElement('button'); up.type='button'; up.className='chip'; up.textContent = p.photo_site ? 'Tukar foto' : 'Muat naik foto';
  const skip = document.createElement('button'); skip.type='button'; skip.className='chip'; skip.textContent='Langkau';
      chips.appendChild(up); chips.appendChild(skip);
      m.querySelector('.bubble').appendChild(chips);

      function done(){ chips.remove(); }

  skip.addEventListener('click', ()=>{ if (state.locked) return; state.locked=true; user('Langkau'); done(); state.locked=false; });
        ...
        fileInput.onchange = async () => {
          ...
          try {
            ...
            if (j && j.ok && j.url){
              ...
              bot('<span class="muted" style="color:#b33;">Muat naik gagal.</span>');
            }
          } catch(_){
            bot('<span class="muted" style="color:#b33;">Ralat muat naik.</span>');
          } finally {
    ...
    // Full name
  const nameAction = await ask('Nama penuh?', { hintHtml: `Semasa: <strong>${escapeHtml(p.full_name||'')}</strong>`, choices:[{label:'Kekal', value:'__keep'}], placeholder:'Masukkan nama penuh baharu' });
    if (nameAction !== '__keep' && nameAction !== '') { updates.full_name = nameAction; }

    // Gender
  const gender = await ask('Jantina?', { choices:[...GENDERS, {label:'Langkau', value:'__skip'}] });
  ...
  const bd = await ask(' Tahun Lahir (YYYY)', { hintHtml:`Semasa: <strong>${escapeHtml(curBDMY||'—')}</strong>`, choices:[{label:'Langkau', value:'__skip'}], placeholder:'cth. 1990 atau 05-07-1990' });
    if (bd !== '__skip') {
  const iso = toISO(bd);
  if (iso && isISO(iso)) updates.birth_date = iso; else if (bd==='') updates.birth_date = null; else bot('<span class="muted" style="color:#b33;">Tarikh tidak sah diabaikan; guna YYYY atau DD-MM-YYYY.</span>');
    }

    // Birth place (optional)
  const bp = await ask('Tempat lahir?', { hintHtml:`Semasa: <strong>${escapeHtml(p.birth_place||'—')}</strong>`, choices:[{label:'Langkau', value:'__skip'}], placeholder:'Bandar, Negara' });
    if (bp !== '__skip') { updates.birth_place = bp || null; }

    // Death block (only if deceased)
    if (isDeceased){
  const curDDMY = toDMY(p.death_date) || (p.death_date||'');
  const dd = await ask('Tahun Meninggal (YYYY)?', { hintHtml:`Semasa: <strong>${escapeHtml(curDDMY||'—')}</strong>`, choices:[{label:'Langkau', value:'__skip'}], placeholder:'cth. 2020' });
      if (dd !== '__skip'){
        const iso = toISO(dd);
  if (iso && isISO(iso)) updates.death_date = iso; else if (dd==='') updates.death_date = null; else bot('<span class="muted" style="color:#b33;">Tarikh tidak sah diabaikan; guna YYYY atau DD-MM-YYYY.</span>');
      }
  const bur = await ask('Tempat pengebumian?', { hintHtml:`Semasa: <strong>${escapeHtml(p.burial_place||'—')}</strong>`, choices:[{label:'Langkau', value:'__skip'}], placeholder:'Tanah perkuburan / Tempat' });
    ...
    if (!isDeceased){
      // Profession (optional)
      // Email (optional)
  const email = await ask('Emel?', { hintHtml:`Semasa: <strong>${escapeHtml(p.email||'—')}</strong>`, choices:[{label:'Langkau', value:'__skip'}], placeholder:'cth. saya@contoh.com' });
      if (email !== '__skip') { updates.email = email || null; }
      // Mobile (optional)
     
    } else {
      // Optional gentle note for deceased persons
  bot('<span class="muted">Medan hubungan disembunyikan untuk individu yang telah meninggal.</span>');
    }
